...
### 10.5 Decisioni prese da Giovanni (06/09/2026) — implementate e verificate dal vivo

1. **Rate mancanti** — decisione: generare solo sui 10 contratti reali confermati, non toccare gli 11 sospetti. Migration `20260906030000`: rate generate (stessa aritmetica di `contract_payments_sync`) sui 10 contratti di Rosella Elia/Antonio Mancuso/Giangianni Flowers. Verificato dal vivo: somme corrette al centesimo su tutti e 10 (es. Renzo e Lucia 16.800€ → 5040/8400/3360).
2. **Gate R1 su "Genera contratto"** — decisione: bloccare come lato cliente. Migration `20260906040000`: `quotes_validate_status_transition` ora richiede, per la transizione a CONVERTITO_IN_CONTRATTO, almeno una voce ACCETTATO se il preventivo ha voci (stessa regola di `quote-accept-sign`) — enforced a livello di trigger, quindi vale per QUALUNQUE chiamante, non solo l'editor. Corretto anche un bug collaterale trovato implementando il fix: `handleCreateContract` non controllava mai l'esito dell'update a CONVERTITO_IN_CONTRATTO, quindi in caso di rifiuto avrebbe mostrato "successo" lasciando un contratto orfano scollegato dallo stato del preventivo; ora l'errore blocca il flusso con un messaggio chiaro e il contratto appena creato viene rimosso (nessun orfano). Verificato dal vivo in entrambe le direzioni: preventivo senza voci accettate → rifiutato (`P0001`); accettata una voce → passa.
