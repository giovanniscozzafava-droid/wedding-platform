...
Deno.serve(async (req) => {
  ...
  const html = emailShell({
    accent: primary,
    eyebrow: 'Addendum da firmare',
    title: a.title ?? 'Addendum al contratto',
    subtitleHtml: `Da <strong>${esc(wpName)}</strong> · variazione ${deltaFmt}`,
    bodyHtml: `<p style="margin:0">Le voci concordate sono cambiate rispetto al contratto firmato. Trovi qui l'atto integrativo (addendum): aprilo e firmalo online, come hai già fatto per il contratto.</p>`,
    cta: { href: link, label: "Apri e firma l'addendum" },
