...
Deno.serve(async (req) => {
  ...
  if (found) {
    const { data: prof } = await admin.from('profiles').select('id, role, full_name, business_name').eq('id', found.id).maybeSingle()
    ...
    const inviterName = inviterProf?.full_name ?? 'Un wedding planner'
