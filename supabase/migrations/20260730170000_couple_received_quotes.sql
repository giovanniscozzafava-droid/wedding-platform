-- La coppia deve vedere TUTTI i preventivi ricevuti per il suo evento (dal fotografo principale +
-- da tutti i professionisti suggeriti), raggruppati per categoria. Riconoscimento evento via la
-- chiave-evento (email cliente + data). Due RPC: elenco (per le cartelle) + dettaglio per-preventivo.

-- 1) Elenco dei preventivi ricevuti per l'evento della coppia (una "cartella" per preventivo).
...
begin
  ...
  return query
    select q.id, coalesce(pr.business_name, pr.full_name) as professionista, pr.subrole, pr.role::text,
           q.status::text, q.total_client, (q.id = v_main) as is_main, q.quote_origin
...
-- 2) Dettaglio di UNO dei preventivi ricevuti (stessa forma di couple_get_quote_for_entry, per quote_id).
...
begin
  ...
  -- Sicurezza: il chiamante e' membro di un evento con la stessa chiave-evento del preventivo (o admin).
