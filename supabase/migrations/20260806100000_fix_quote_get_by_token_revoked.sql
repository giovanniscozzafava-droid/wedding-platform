-- REGRESSIONE SICUREZZA: quote_get_by_token (lettura pubblica del preventivo via link) controllava
-- solo la SCADENZA del token (is_token_valid), non la REVOCA. La revoca (revoke_share_token) fa
-- `update quotes set token_revoked_at = now()` SENZA azzerare access_token; accept/reject_by_token
-- controllano `token_revoked_at is null`, e p7_stress_fixes l'aveva aggiunto anche alla lettura — ma
-- quote_token_hide_markup (20260605210000) ha ri-creato la funzione perdendo il check. Risultato: un
-- link di preventivo REVOCATO dal professionista restava visibile (finché non scaduto). Qui
-- ripristiniamo il controllo di revoca. (Whitelist campi e is_token_valid restano invariati.)
