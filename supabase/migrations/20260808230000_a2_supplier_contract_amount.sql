-- ============================================================================
-- A2/M1: create_supplier_contract calcolava total_amount = sum(line_client) di
-- TUTTE le voci del fornitore.
--  A2: per un contratto SUPPLIER_WP (fornitore↔capostipite) l'importo è quello
--      che il capostipite DEVE al fornitore = il COSTO (line_cost), non il prezzo
--      cliente (line_client = costo + markup del capostipite): usare line_client
--      svelava il margine al fornitore ed era formalmente errato. Per
--      SUPPLIER_CLIENT (fornitore↔cliente) il fornitore fattura al cliente →
--      resta line_client.
--  M1: sommava anche voci non vendute. Ora conta solo le voci vendute (ACCETTATO
--      se c'è selezione per-voce, altrimenti tutte le non-RIFIUTATO — come R1).
...
begin
  ...
  -- A2/M1: importo per il fornitore. SUPPLIER_WP → COSTO (line_cost); SUPPLIER_CLIENT
