-- Scopri: l'esclusione dei CONCORRENTI diretti (un fotografo non vede altri fotografi, una WP non
-- vede altre WP…) NON deve valere per ADMIN e STAFF (founder). Un admin/staff, anche se ha un
-- subrole (es. account fornitore/fotografo di prova), deve poter vedere TUTTI i fornitori.
-- Aggiungo un bypass `is_admin/is_support_staff` in entrambe le funzioni di Scopri, lasciando
-- invariata la regola per gli utenti normali.
...
language sql stable security definer set search_path = public as $$
  ...
  where p.role in ('WEDDING_PLANNER','LOCATION')
    ...
    and p.id <> coalesce(auth.uid(), '00000000-0000-0000-0000-000000000000'::uuid)
    -- admin/staff (founder): niente filtro anti-concorrenti → vede tutti.
    and (
      exists (select 1 from profiles priv where priv.id = auth.uid()
              and (priv.role = 'ADMIN' or coalesce(priv.is_support_staff, false)))
      or not exists (
