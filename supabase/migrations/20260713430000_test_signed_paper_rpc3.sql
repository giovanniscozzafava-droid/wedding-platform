-- TEST e2e finale RPC contract_mark_signed_paper: preventivo ACCETTATO/CONVERTITO senza contratto. Self-cleaning.
do $$
...
begin
  ...
  if v_owner is null then raise notice 'TEST RPC3 CARTACEO: nessun preventivo ACCETTATO libero, salto (funzione comunque deployata)'; return; end if;
  begin
    insert into public.contracts (owner_id, quote_id, title, client_name, total_amount, status, access_token)
      values (v_owner, v_qid, '__TEST_RPC3_CARTACEO__', 'Mario Rossi', 1000, 'INVIATO', gen_random_uuid())
    ...
    if v_denied and v_ok and v_status = 'FIRMATO' and v_meth = 'CARTACEO' then
      raise notice 'TEST RPC3 CARTACEO: OK — estraneo respinto, owner firma → FIRMATO method=CARTACEO';
    else
      raise notice 'TEST RPC3 CARTACEO: ESITO INATTESO denied=% ok=% status=% method=%', v_denied, v_ok, v_status, v_meth;
    end if;
  exception when others then
    if v_cid is not null then delete from public.contracts where id = v_cid; end if;
    raise notice 'TEST RPC3 CARTACEO: salto (%).', SQLERRM;
  end;
end $$;
