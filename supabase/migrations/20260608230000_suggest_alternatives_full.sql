-- ============================================================================
-- Quando il professionista richiesto è OCCUPATO/bloccato per quella data, il
-- form propone 2 colleghi dello STESSO settore, ISCRITTI e DISPONIBILI, con i
-- loro CONTATTI. Da qui il cliente riparte (il loro profilo ha a sua volta il
-- form). Questa versione include i contatti per l'email ed è sempre attiva.
-- ============================================================================
...
begin
  ...
  select coalesce(jsonb_agg(x), '[]'::jsonb) into v_res from (
    select jsonb_build_object(
             'id', c.id, 'name', coalesce(c.business_name, c.full_name), 'slug', c.slug,
             'subrole', c.subrole, 'city', c.city, 'phone', c.phone, 'role', c.role
           ) as x
