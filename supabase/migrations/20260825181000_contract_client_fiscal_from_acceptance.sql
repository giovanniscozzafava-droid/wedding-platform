-- ============================================================================
-- build_contract_sections — il fiscale del CLIENTE veniva letto SOLO da
-- supplier_clients (via quotes.direct_client_id). Per un capostipite
-- WEDDING_PLANNER/LOCATION (contratto CLIENT_WP) direct_client_id è NULL →
-- il contratto stampava "C.F. [da completare]" ANCHE se il cliente aveva
-- fornito i dati fiscali accettando/firmando il preventivo (snapshot in
-- quote_acceptances). Fix: leggere il fiscale del cliente dall'ultima
-- quote_acceptances (come fa già contract-ai-draft), con fallback a
-- supplier_clients. Anche il NOME del cliente ora ricade su signer_name.
-- (Risolve pure la contraddizione con il blocco "Committente" del PDF, che
--  legge i dati reali: ora la sezione "Premesse" combacia.)
-- ============================================================================
create or replace function public.build_contract_sections(p_quote_id uuid)
...
declare
  ...
  v_total        numeric;
  v_dep_pct      numeric; v_sec_pct numeric; v_bal_pct numeric;
  v_r1 numeric; v_r2 numeric; v_r3 numeric;
  v_sc record; v_acc record; v_prov_fiscal text; v_cli_fiscal text; v_location text; v_items_detail text;
  v_cli_biz text; v_cli_vat text; v_cli_cf text; v_cli_addr text; v_cli_city text; v_cli_prov text; v_cli_zip text;
begin
  ...
  -- Dati per un contratto valido: fiscale del cliente dall'ultima accettazione
  -- (lo scrive lui firmando), fallback rubrica fornitore (direct_client_id).
  select sc.* into v_sc from supplier_clients sc where sc.id = v_quote.direct_client_id;
  select qa.* into v_acc from quote_acceptances qa
    where qa.quote_id = p_quote_id order by qa.created_at desc limit 1;

  v_prov_fiscal := nullif(concat_ws(', ',
      case when nullif(trim(coalesce(v_owner.vat_number,'')),'')  is not null then 'P.IVA ' || v_owner.vat_number end,
      case when nullif(trim(coalesce(v_owner.fiscal_code,'')),'') is not null then 'C.F. '  || v_owner.fiscal_code end), '');

  -- Coalesce accettazione → anagrafica per ogni campo fiscale del cliente.
  v_cli_biz  := coalesce(nullif(trim(v_acc.client_business_name),''), nullif(trim(v_sc.business_name),''));
  v_cli_vat  := coalesce(nullif(trim(v_acc.client_vat_number),''),    nullif(trim(v_sc.vat_number),''));
  v_cli_cf   := coalesce(nullif(trim(v_acc.client_fiscal_code),''),   nullif(trim(v_sc.fiscal_code),''));
  v_cli_addr := coalesce(nullif(trim(v_acc.client_address),''),       nullif(trim(v_sc.address),''));
  v_cli_city := coalesce(nullif(trim(v_acc.client_city),''),          nullif(trim(v_sc.city),''));
  v_cli_prov := coalesce(nullif(trim(v_acc.client_province),''),      nullif(trim(v_sc.province),''));
  v_cli_zip  := coalesce(nullif(trim(v_acc.client_zip),''),           nullif(trim(v_sc.zip),''));

  v_cli_fiscal := nullif(concat_ws(', ',
      case when v_cli_biz is not null then 'ragione sociale ' || v_cli_biz end,
      case when v_cli_vat is not null then 'P.IVA ' || v_cli_vat end,
      case when v_cli_cf  is not null then 'C.F. '  || v_cli_cf else 'C.F. [da completare]' end,
      nullif(trim(concat_ws(' ', v_cli_addr, v_cli_city,
           case when v_cli_prov is not null then '(' || v_cli_prov || ')' end, v_cli_zip)), '')), '');
  v_location := nullif(trim(coalesce(v_quote.event_location, '')), '');
  select string_agg(
      '• ' || qi.name_snapshot
      || case when coalesce(qi.quantity,1) <> 1 then ' (quantita ' || trim(to_char(qi.quantity,'FM999990.##')) || coalesce(' ' || nullif(lower(qi.unit_snapshot::text),''),'') || ')' else '' end
      || ' — € ' || to_char(coalesce(qi.line_client,0), 'FM999G999G990D00')
      || case when nullif(trim(coalesce(qi.description_snapshot,'')),'') is not null then E'\n   ' || qi.description_snapshot else '' end,
      E'\n' order by qi.sort_order)
    into v_items_detail
    from quote_items qi
    where qi.quote_id = p_quote_id
      and (case when coalesce(v_quote.total_client_selected,0) > 0 then qi.client_decision = 'ACCETTATO' else true end);
  ...
  v_dep_pct := coalesce(v_owner.pay_deposit_pct, 30);
  v_sec_pct := coalesce(v_owner.pay_second_pct, 50);
  v_bal_pct := 100 - v_dep_pct - v_sec_pct;
  v_r1 := round(v_total * v_dep_pct / 100.0, 2);
  v_r2 := round(v_total * v_sec_pct / 100.0, 2);
  v_r3 := round(v_total - v_r1 - v_r2, 2);
  ...
  v_provider_address := coalesce(nullif(concat_ws(' - ', v_owner.address, v_owner.city, v_owner.country), ''), '[indirizzo da completare]');
  v_provider_pec := coalesce(nullif(trim(v_owner.pec_email), ''), '[email PEC del Prestatore da inserire]');

  v_client_name := coalesce(nullif(trim(v_quote.client_name), ''), nullif(trim(v_acc.signer_name), ''), nullif(trim(v_sc.full_name), ''), '[Cliente da identificare]');
  ...
  return jsonb_build_array(
    jsonb_build_object(
      ...
      'body', concat(
        ...
        'TRA', E'\n',
        v_provider_name, ' (di seguito anche il "Prestatore" o "Parte fornitrice"), ', v_provider_role, ', con sede in ', v_provider_address, coalesce(', ' || v_prov_fiscal, ''), ', PEC: ', v_provider_pec, ',', E'\n\n',
        'E', E'\n',
        v_client_name, ' (di seguito anche il "Committente" o "Parte committente"), ', coalesce(v_cli_fiscal || ', ', ''), 'domiciliato/a ai fini del presente contratto presso ', v_client_email, ',', E'\n\n',
        'Premesso che:', E'\n',
    ...
    jsonb_build_object(
      ...
      'body', concat(
        ...
        '1.2 Le prestazioni saranno erogate in occasione dell''Evento (', v_event_label, ') previsto per il giorno ', v_event_date, coalesce(' presso ' || v_location, ''), '. Eventuali variazioni della data dovranno essere concordate per iscritto tra le Parti e potranno comportare adeguamenti del corrispettivo.', E'\n\n',
    ...
    jsonb_build_object(
      'heading', 'Art. 1-bis — Dettaglio dei servizi',
      'type', 'SERVIZI',
      'body', concat(
        'Elenco dettagliato delle prestazioni oggetto del presente contratto, come da Preventivo:', E'\n\n',
        coalesce(v_items_detail, '[nessuna voce selezionata]'), E'\n\n',
        'Totale dei servizi: € ', v_total_str, '.'
      )
    ),
    jsonb_build_object(
      ...
      'body', concat(
        ...
        '   a) acconto del ', trim(to_char(v_dep_pct, 'FM990D99')), '% pari a € ', to_char(v_r1, 'FM999G999G990D00'), ' al momento della sottoscrizione del presente contratto, a titolo di caparra confirmatoria ai sensi dell''art. 1385 c.c.;', E'\n',
        '   b) ulteriore ', trim(to_char(v_sec_pct, 'FM990D99')), '% pari a € ', to_char(v_r2, 'FM999G999G990D00'), ' entro 60 (sessanta) giorni precedenti la data dell''Evento;', E'\n',
        '   c) saldo del restante ', trim(to_char(v_bal_pct, 'FM990D99')), '% pari a € ', to_char(v_r3, 'FM999G999G990D00'), ' entro 7 (sette) giorni precedenti la data dell''Evento.', E'\n\n',
    ...
    jsonb_build_object(
      ...
      'type', 'TERMINE',
      'body', concat(
        '3.1 Le prestazioni saranno eseguite il giorno ', v_event_date, ' presso ', coalesce(v_location, 'il luogo dell''Evento concordato tra le Parti e indicato nel Preventivo'), '.', E'\n\n',
