-- FIX counter pubblico "eventi gestiti": total_events contava TUTTE le calendar_entries dell'owner,
-- incluse RIFIUTATA/CANCELLATA e ogni IN_TRATTATIVA (creata automaticamente a OGNI preventivo
-- inviato) → numero gonfiato sulla vetrina pubblica. Contiamo solo gli eventi CONFERMATA (il vero
-- track record). Resto della funzione identico alla 20260619150000 (con cover_image_url).
...
begin
  ...
  -- SOLO eventi confermati = track record reale (no bozze/in-trattativa/annullati).
  select count(*) into v_total_events from calendar_entries where owner_id = v_wp.id and status = 'CONFERMATA';
