-- ============================================================================
-- HOTFIX: my_quote_conflict_alerts() ritornava 42703 in produzione perché
-- referenziava profiles.email (non esiste — l'email vive in auth.users).
-- Conseguenza: ConflictAlertsBanner silenziosamente rotto, alert mai mostrati.
--
-- Fix: coalesce su business_name + full_name, niente email leaked all'altro
-- lato (era comunque un mini-leak in caso di nome mancante).
-- ============================================================================
...
begin
  ...
  select
    ...
    coalesce(nullif(trim(p.business_name), ''), nullif(trim(p.full_name), ''), 'Utente Planfully') as other_owner_name,
