-- FILO CON MEMORIA, CHE LEGGE LA REALTÀ (Giovanni, 02/09/2026):
--   «Filo deve essere più attivo e cosciente e leggere la realtà. Dice ogni giorno le
--    stesse cose. Bisogna interagire, tipo "ho letto, ho capito", altrimenti ripete
--    ogni giorno le stesse cose passivamente.»
--
-- Il difetto era strutturale: filo_brief ricalcolava ogni volta gli stessi consigli
-- senza memoria di averli già detti, con chiavi generiche ('hot_quote') che non
-- distinguevano un cliente dall'altro, e guardava solo lo stato "fermo" del business
-- (quanti preventivi, quanti servizi). Non vedeva niente di quello che era SUCCESSO:
-- chi è entrato nell'evento, chi ha aperto il preventivo, chi ha risposto all'ultimatum.
--
-- Tre cose cambiano:
--   1. MEMORIA: per ogni utente e ogni segnale, Filo ricorda quando l'ha detto la prima
--      volta e se l'utente ha risposto "ho capito". Un "ho capito" lo spegne finché la
--      realtà non cambia: la chiave porta con sé un'impronta di stato (es. quante volte
--      il cliente ha aperto). Se l'impronta cambia, il segnale torna, perché è una
--      notizia nuova, non una ripetizione.
--   2. NOTIZIE: accanto ai consigli, un blocco "Filo ha letto" con i fatti delle ultime
--      24-48 ore, per singolo cliente. Sparisce da solo quando invecchia.
--   3. CHIAVI PER CLIENTE: 'hot_quote:<id>' e non 'hot_quote'. Così "ho capito" su un
--      cliente non zittisce Filo su un altro.

-- ───────────────────────── memoria ─────────────────────────
create table if not exists public.filo_signal_state (
  user_id       uuid not null references public.profiles(id) on delete cascade,
  key           text not null,
  first_seen_at timestamptz not null default now(),
  last_seen_at  timestamptz not null default now(),
  state         text,               -- impronta dello stato l'ultima volta che l'ha mostrato
  acked_at      timestamptz,
  ack_state     text,               -- impronta al momento dell'"ho capito"
  snooze_until  timestamptz,
  primary key (user_id, key)
);
alter table public.filo_signal_state enable row level security;
drop policy if exists filo_state_own on public.filo_signal_state;
create policy filo_state_own on public.filo_signal_state for select using (user_id = auth.uid());

comment on table public.filo_signal_state is
  'Memoria di Filo: cosa ha già detto a chi, e cosa l''utente ha segnato come capito.';

-- "Ho capito": spegne il segnale per p_days giorni, MA se l'impronta di stato cambia
-- nel frattempo il segnale torna (è successo qualcosa di nuovo).
create or replace function public.filo_ack(p_key text, p_state text default null, p_days integer default 7)
returns jsonb language plpgsql security definer set search_path = public as $$
declare v_uid uuid := auth.uid();
begin
  if v_uid is null then return jsonb_build_object('error','auth'); end if;
  if coalesce(trim(p_key),'') = '' then return jsonb_build_object('error','bad_key'); end if;
  insert into filo_signal_state (user_id, key, state, acked_at, ack_state, snooze_until)
  values (v_uid, p_key, p_state, now(), p_state, now() + make_interval(days => greatest(1, coalesce(p_days,7))))
  on conflict (user_id, key) do update
     set acked_at = now(), ack_state = excluded.ack_state, snooze_until = excluded.snooze_until;
  return jsonb_build_object('ok', true);
end$$;
revoke all on function public.filo_ack(text, text, integer) from public;
grant execute on function public.filo_ack(text, text, integer) to authenticated;

-- "Ho letto tutto": stessa cosa per una lista [{key,state,days}].
create or replace function public.filo_ack_many(p_items jsonb)
returns jsonb language plpgsql security definer set search_path = public as $$
declare v_uid uuid := auth.uid(); it jsonb; n int := 0;
begin
  if v_uid is null then return jsonb_build_object('error','auth'); end if;
  for it in select * from jsonb_array_elements(coalesce(p_items, '[]'::jsonb)) loop
    if coalesce(it->>'key','') = '' then continue; end if;
    insert into filo_signal_state (user_id, key, state, acked_at, ack_state, snooze_until)
    values (v_uid, it->>'key', it->>'state', now(), it->>'state',
            now() + make_interval(days => greatest(1, coalesce((it->>'days')::int, 7))))
    on conflict (user_id, key) do update
       set acked_at = now(), ack_state = excluded.ack_state, snooze_until = excluded.snooze_until;
    n := n + 1;
  end loop;
...
revoke all on function public.filo_ack_many(jsonb) from public;
grant execute on function public.filo_ack_many(jsonb) to authenticated;

-- ───────────────────────── piccoli aiuti di lingua ─────────────────────────
create or replace function public.filo_ago(p_ts timestamptz)
returns text language sql stable as $$
  select case
    when p_ts is null then ''
    when now() - p_ts < interval '2 minutes' then 'adesso'
    when now() - p_ts < interval '1 hour' then floor(extract(epoch from (now()-p_ts))/60)::int || ' minuti fa'
    when now() - p_ts < interval '2 hours' then 'un''ora fa'
    when now() - p_ts < interval '24 hours' then floor(extract(epoch from (now()-p_ts))/3600)::int || ' ore fa'
    ...
    when now() - p_ts < interval '7 days' then floor(extract(epoch from (now()-p_ts))/86400)::int || ' giorni fa'
    else 'il ' || to_char(p_ts at time zone 'Europe/Rome', 'DD/MM')
  end
$$;

create or replace function public.filo_durata(p_sec integer)
returns text language sql immutable as $$
  select case
    when coalesce(p_sec,0) < 60 then p_sec || ' s'
    when p_sec < 3600 then (p_sec/60) || ' min'
    else (p_sec/3600) || 'h ' || lpad(((p_sec % 3600)/60)::text, 2, '0')
  end
$$;

-- "10.%" no: la virgola decimale solo se serve, e i separatori all'italiana.
create or replace function public.filo_pct(p numeric)
returns text language sql immutable as $$
  select replace(rtrim(rtrim(to_char(coalesce(p,0),'FM990D99'),'0'),'.'), '.', ',')
$$;
create or replace function public.filo_eur(p numeric)
returns text language sql immutable as $$
  select replace(replace(replace(to_char(coalesce(p,0),'FM999G999G990D00'),',','#'),'.',','),'#','.') || ' €'
$$;

create or replace function public.filo_sezione(p_s text)
returns text language sql immutable as $$
  select case p_s
    when 'foto' then 'sulle foto' when 'preventivo' then 'sul preventivo'
    when 'contratto' then 'sul contratto' when 'overview' then 'in panoramica'
    when 'video' then 'sui video' when 'invitati' then 'sugli invitati'
    when 'tavoli' then 'sui tavoli' when 'chat' then 'nei messaggi'
    when 'audio' then 'sugli auguri audio' when 'guestbook' then 'sul guestbook'
    else 'su ' || p_s end
$$;

-- ───────────────────────── il brief ─────────────────────────
-- Da STABLE a VOLATILE: ora scrive la propria memoria (first_seen/last_seen).
