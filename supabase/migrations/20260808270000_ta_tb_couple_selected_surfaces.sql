-- ============================================================================
-- T-A/T-B: le RPC del portale cliente/coppia non esponevano total_client_selected,
-- cosi la tab "I miei fornitori" (ClientProfessionalsView) e il primo load coppia
-- mostravano una somma grezza / il totale pieno, diversa dal totale vincolante.
-- Aggiungiamo total_client_selected a client_portal_overview e
-- couple_get_quote_for_entry. Funzioni estratte byte-per-byte da 20260808150000.
-- ============================================================================
    ...
    select jsonb_build_object(
      ...
      'quotes', (
        select coalesce(jsonb_agg(jsonb_build_object(
          ...
          'total_client_selected', q.total_client_selected,
          ...
          'items', (
            select coalesce(jsonb_agg(jsonb_build_object(
              ...
              'category', sc.name,
              'photo', case when public.quote_item_is_blind(qi, coalesce(pr.capostipite_sale_mode, 'BUNDLE')) then null
                        else (select ph.thumbnail_url from public.service_photos ph
                        where ph.service_id = qi.service_id order by ph.sort_order limit 1) end
...
begin
  ...
  select jsonb_agg(jsonb_build_object(
           ...
           'category', sc.name,
           'photo', case when public.quote_item_is_blind(qi, v_mode) then null
                    else (select ph.thumbnail_url from public.service_photos ph
                     where ph.service_id = qi.service_id order by ph.sort_order limit 1) end
