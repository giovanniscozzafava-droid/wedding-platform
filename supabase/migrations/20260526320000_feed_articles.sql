-- ============================================================================
-- POSTS ARTICLES — il feed evolve da "short post IG-style" a "post + articoli
-- LinkedIn-style". I post brevi (SHORT) restano come oggi (testo + foto).
-- Si aggiungono articoli (ARTICLE) con titolo, cover, body HTML lungo
-- editabile via tiptap, indicizzabili come pagina dedicata /feed/post/:id.
-- ============================================================================

alter table posts
  add column if not exists post_type     text not null default 'SHORT' check (post_type in ('SHORT','ARTICLE')),
...
-- Estendiamo feed_home per esporre i nuovi campi (return type cambia, drop & create)
...
-- Trending include nuovi campi (return type cambia, drop & create)
  ...
  scored as (
    select
      ...
      (
        ...
        * case when p.post_type = 'ARTICLE' then 1.3 else 1.0 end  -- gli articoli pesano di più
      ) as trending_score
    from posts p
    join profiles pr on pr.id = p.author_id
    left join calendar_entries ce on ce.id = p.event_id
...
-- RPC: articolo singolo per pagina /feed/post/:slug (pubblica con visibility check)
...
begin
  ...
  -- Visibility public-only on this endpoint (cosi e' indicizzabile)
  if v_post.visibility <> 'PUBLIC' then
    -- Permettiamo accesso a autore e admin
    if v_post.author_id is distinct from auth.uid() and not is_admin() then
      return null;
    end if;
  end if;
