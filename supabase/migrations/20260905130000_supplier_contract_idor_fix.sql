-- ============================================================================
-- IDOR trovato dal collaudo dal vivo del 05/09/2026 (stress-test fornitore):
-- create_supplier_contract autorizzava chiunque a chiamare la RPC passando
-- SE STESSO come p_supplier_id, su QUALUNQUE p_entry_id — senza verificare
-- alcuna relazione reale (collaborazione attiva, o anche solo una voce sua
-- sul preventivo di quell'evento). Un fornitore poteva quindi auto-generarsi
-- un mini-contratto su un evento a cui non ha mai partecipato e leggerne i
-- dati fiscali del capostipite proprietario (nome, CF, P.IVA, indirizzo).
-- Bug pre-esistente (il controllo non è cambiato dal fix di stamattina sulla
-- intestazione del contratto, 20260905100000): oggi prende nuova rilevanza
-- perché quei dati fiscali sono proprio quelli del capostipite.
--
-- Fix: se il chiamante non è il capostipite (owner dell'evento) né admin,
-- deve avere davvero almeno una voce come fornitore su QUESTO preventivo —
-- non basta dichiararsi "sono io" via parametro.
-- ============================================================================
...
begin
  ...
  if not (v_entry.owner_id = auth.uid() or is_admin()) then
    -- Non e' il capostipite: deve essere davvero il fornitore indicato, CON
    -- una voce sua su questo preventivo (prova di una relazione reale sulla
    -- coppia entry/fornitore, non solo "p_supplier_id = auth.uid()").
