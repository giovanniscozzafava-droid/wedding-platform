...
begin
  ...
  for t in select unnest(array['entry_status','ceremony_status','evento_stato','quote_status','quote_margin_mode','contract_party_kind','guest_age_group']) loop
    ...
    raise notice 'ENUM % => %', t, coalesce(c,'(non enum)');
  end loop;
  select pg_get_constraintdef(oid) into c from pg_constraint where conrelid='public.calendar_entries'::regclass and contype='c' and pg_get_constraintdef(oid) ilike '%business_model%' limit 1;
