-- album_catalog_for_entry: ritorna TUTTI i cataloghi PDF + TUTTI i modelli (card da PDF + card
-- manuali) del fotografo dell'evento, con prezzo/foto/opzioni (NON il costo, che è interno).
-- Retrocompat: 'catalog' + 'hotspots' = primo PDF, come prima.
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object('id', id, 'name', name, 'pdf_path', pdf_path, 'page_count', page_count)
  ...
  return jsonb_build_object('ok', true, 'studio', v_studio,
    ...
    'catalog', case when v_cat0 is null then null else jsonb_build_object(
        ...
        'page_count', (v_cat0->>'page_count')::int, 'owner_id', v_owner, 'studio', v_studio) end,
