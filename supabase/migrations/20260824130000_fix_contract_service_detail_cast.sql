-- Contratto: importa il DETTAGLIO servizi (voci accettate) come 'Art. 1-bis'.
...
declare
  ...
  v_sc record; v_prov_fiscal text; v_cli_fiscal text; v_location text; v_items_detail text;
begin
  ...
  -- Dati per un contratto valido: cliente in rubrica fornitore (direct_client_id),
  -- dati fiscali delle parti, luogo dell'Evento.
  ...
  v_cli_fiscal := nullif(concat_ws(', ',
      case when nullif(trim(coalesce(v_sc.business_name,'')),'') is not null then 'ragione sociale ' || v_sc.business_name end,
      case when nullif(trim(coalesce(v_sc.vat_number,'')),'')    is not null then 'P.IVA ' || v_sc.vat_number end,
      case when nullif(trim(coalesce(v_sc.fiscal_code,'')),'')   is not null then 'C.F. '  || v_sc.fiscal_code else 'C.F. [da completare]' end,
      nullif(trim(concat_ws(' ', v_sc.address, v_sc.city,
           case when nullif(trim(coalesce(v_sc.province,'')),'') is not null then '(' || v_sc.province || ')' end, v_sc.zip)), '')), '');
  ...
  -- #2: piano pagamenti del professionista (default 30/50/20). Ultima rata a
  -- residuo così le tre rate sommano sempre al totale (#11).
