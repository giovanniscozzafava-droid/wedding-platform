-- ════════════════════════════════════════════════════════════════════════════
-- PILASTRO 3 — BLIND UNIFICATO (decisione: "nascosto ovunque, voce per voce").
-- Il "blind" (in pacchetto il cliente non vede quali fornitori usi) era applicato
-- SOLO al percorso pubblico quote_get_by_token. Le due proiezioni CLIENTE autenticate
-- lo ignoravano → CAP-1 (leak nome fornitore appena il cliente fa login) + il vecchio
-- masking coarse via business_model='GLOBAL'. E la vetrina recensioni pubblica
-- esponeva il titolo evento grezzo (spesso "Matrimonio <cognome>") ad anon → RD-2.
-- Unifichiamo tutto sull'unica regola canonica: quote_item_is_blind(qi, sale_mode)
-- = coalesce(supplier_blind, sale_mode='BUNDLE'). Per-voce resta possibile mostrare
-- (supplier_blind=false) anche in pacchetto.
-- Ricreazione VERBATIM delle funzioni + sola guardia blind (nessun'altra logica tocca).
-- Nota: il fallback subrole in quote_get_by_token (CAP-4) è chiuso in una migration
-- separata (ricrea quella funzione).
-- ════════════════════════════════════════════════════════════════════════════

-- 1) client_portal_overview — CAP-1: nasconde il NOME del fornitore sulle voci blind.
        ...
        select coalesce(jsonb_agg(jsonb_build_object(
          ...
          'items', (
            select coalesce(jsonb_agg(jsonb_build_object(
              ...
              -- BLIND: se la voce è cieca (o senza fornitore) non riveliamo il nome.
              'supplier', case
                when public.quote_item_is_blind(qi, coalesce(pr.capostipite_sale_mode, 'BUNDLE')) or qi.supplier_id is null
                then null
                else coalesce(sp.business_name, sp.full_name)
              end,
...
-- 2) couple_get_quote_for_entry — CAP-1: masking per-voce sulla regola canonica
--    (capostipite_sale_mode + supplier_blind) invece del vecchio business_model='GLOBAL'.
...
begin
  ...
  -- BLIND canonico: modalità di vendita del capostipite (owner del preventivo).
  ...
  select jsonb_agg(jsonb_build_object(
           ...
           -- voce cieca → nessun supplier_id verso la coppia.
           'supplier_id', case when public.quote_item_is_blind(qi, v_mode) then null else qi.supplier_id end,
           'category', sc.name,
           'photo', (select ph.thumbnail_url from public.service_photos ph
                     where ph.service_id = qi.service_id order by ph.sort_order limit 1)
...
-- 3) list_user_reviews — RD-2: il titolo evento grezzo (spesso "Matrimonio <cognome>")
--    NON deve finire sulla vetrina pubblica anon. Lo mascheriamo (null).
...
as $$
  select
    ...
    p.brand_logo_url,
    p.slug,
    null::text as entry_title   -- RD-2: mai il titolo evento (PII coppia) su superficie pubblica
