...
type Connection = { company_id: string; company_name: string | null }
...
export function FattureInCloudCard() {
  ...
  const connQ = useQuery({
    ...
    enabled: !!user,
    queryFn: async () => {
      const { data } = await supabase.from('fic_connections').select('company_id, company_name').eq('professional_id', user!.id).maybeSingle()
