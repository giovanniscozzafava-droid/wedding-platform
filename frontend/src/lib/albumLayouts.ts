...
export type SavedLayout = { id: string; name: string; n: number; frames: Frame[] }
...
export function saveLayout(name: string, frames: Frame[]): SavedLayout[] {
  const item: SavedLayout = { id: uid(), name: name.trim() || `Layout ${frames.length} foto`, n: frames.length, frames: frames.map((f) => ({ ...f })) }
