...
import { LayoutDashboard, Users, Zap, LifeBuoy, Loader2, Search, Power } from 'lucide-react'
...
export default function AdminPage() {
  ...
  useEffect(() => { if (tab === 'team') void loadUsers() /* eslint-disable-next-line */ }, [tab])
    ...
    <div className="min-h-full">
      <div className="max-w-5xl mx-auto px-6 sm:px-10 py-10">
        <PageHeader eyebrow="Staff" title="Pannello Admin"
          description="Gestisci team, funnel e tieni d'occhio i numeri della piattaforma." />

        <div className="flex gap-1 mb-6 overflow-x-auto">
          {([['overview', 'Panoramica', LayoutDashboard], ['team', 'Team & staff', Users], ['funnel', 'Funnel', Zap]] as const).map(([v, l, Icon]) => (
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            ...
            <Card className="p-5">
              <h3 className="font-display text-lg mb-3">Preventivi per stato</h3>
              <div className="flex flex-wrap gap-2">
                {Object.entries(ov.quotes_by_status).map(([s, n]) => (
                  <span key={s} className="text-xs px-3 py-1 rounded-full" style={{ background: 'rgb(var(--bg-sunken))' }}>{s}: <strong>{n}</strong></span>
                ))}
              </div>
            </Card>
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs text-[rgb(var(--fg-muted))]">Attiva lo <strong>staff assistenza</strong> per chi deve gestire i ticket (resta col suo ruolo, niente poteri extra).</p>
            {users.map((u) => (
          ...
          <Card className="p-6">
            <div className="flex items-start justify-between gap-4">
              ...
              <span className="text-xs px-3 py-1 rounded-full shrink-0"
                style={{ background: ov.funnel_active ? 'rgb(var(--sage-100))' : 'rgb(var(--bg-sunken))', color: 'rgb(var(--fg-muted))' }}>
                {ov.funnel_active ? 'Acceso' : 'In pausa'}
              </span>
            </div>
            <div className="mt-5">
...
function Stat({ label, value, sub }: { label: string; value: number; sub?: string }) {
  return (
    <Card className="p-4">
      ...
      <p className="font-display text-2xl tabular-nums mt-0.5">{value}</p>
